...
public class UsingSpecialOps {
	...
	public static void main(final String... args) throws Exception {
		...
		// As explained in the very beginning, ops can be implemented as
		// Computer, Function, or Inplace. Additionally, there are ops that are
		// hybrids of those types. There is, for example, a meanOp that can be
		// used as a computer (by providing a reference to the output) or as a
		// function (returning the output).

		log.info("================ HYBRID OPS =========================\n");
		UnaryHybridCF<IterableInterval<DoubleType>, DoubleType> meanOp =
				Hybrids.unaryCF(ops, Ops.Stats.Mean.class, new DoubleType(), sinusoid);

		DoubleType meanOfInput = new DoubleType(0);
		// Use the HybridOp as a ComputerOp: store result in output reference.
		meanOp.compute1(sinusoid, meanOfInput);
		log.info("meanOp(input, meanOfInput) -- meanOfInput = "+ meanOfInput);
		// Use the HybridOp as a FunctionOp: return result as a new object.
		meanOfInput = meanOp.compute1(sinusoid);
		log.info("meanOfInput = meanOp(input) -- meanOfInput = "+ meanOfInput);
		...
		log.info("================ CHAINING OPS ================\n");

		// Some ops accept other ops as arguments, forming chains or trees of
		// ops. Examples include the loop op that executes an op several times
		// or the map op that applies an op to each element of the input.

		// LOOP OP
		// The loop op executes an op for the predefined number of iterations:

		log.info("--------- Loop op --------\n");
		int iterations = 4;
		DoubleType in = new DoubleType(10.0d);

		// Get an instance of the Add op that adds 5.0 to the input value inplace
		UnaryInplaceOp<? super DoubleType, DoubleType> add5 = Inplaces.unary(
				ops, Ops.Math.Add.class, DoubleType.class,
				new DoubleType(5.0d));
		UnaryInplaceOp<? super DoubleType, DoubleType> addLoop = Inplaces
				.unary(ops, Ops.Loop.class, in, add5, iterations);
		// Add 5 to 'in' each time through the loop; store value in 'in'.
		addLoop.mutate(in);
		log.info("loop(in=10, add5, iterations=4)");
		...
		log.info("-------- Map op --------\n");
		IterableInterval<DoubleType> output = ij.op().create().img(sinusoid);

		// The map op calls add5.mutate() on each pixel of the input. It does so
		// by reusing the previously obtained add5 instance. Hence, there are no
		// additional calls into the Ops framework.
		UnaryComputerOp<IterableInterval<DoubleType>, IterableInterval<DoubleType>> mapOp =
			IIs.computer(ops, Ops.Map.class, sinusoid, add5);
		mapOp.compute1(sinusoid, output);

		log.info("-------- Input mean: " + meanOp.compute1(sinusoid) +
